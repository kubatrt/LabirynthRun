...
public class DFS : MonoBehaviour 
{
	...
	class MazeCell
	{
		...
		public MazeCell(float x, float y, int i) {
		...
		{
			return string.Format("\t Cell({6}) [{0},{1}] walls N: {2} S: {3} E: {4} W: {5}", 
			                     position.x, position.y, north, south, east, west, index);
		}
	}
	...
	void Generation()
	{
		...
		for(int y = 0; y < sizeY; y++)
		{
			...
			{
				mazeCells[x,y] = new MazeCell(x, y, i++);	// set position related to place in array
		...
		//Debug.Log ( "START + " + currentCell.ToString());
	...
	List<MazeCell> GetNeighbors2(MazeCell cell)
	{
		List<MazeCell> neighbors = new List<MazeCell>();
	...
	private void OnDrawGizmos()
	{
		...
		for(int y = 0; y < sizeY; y++)
		{
			for(int x = 0; x < sizeX; x++)
			{
				...
				Gizmos.color = Color.cyan;
				if (cell.north && cellStack.Contains(cell.position) )
					Gizmos.DrawLine(topLeft, topRight);
				if (cell.south && cellStack.Contains(cell.position))
					Gizmos.DrawLine(bottomLeft, bottomRight);
				if (cell.east && cellStack.Contains(cell.position))
					Gizmos.DrawLine(topRight, bottomRight);
				if (cell.west && cellStack.Contains(cell.position))
					Gizmos.DrawLine(topLeft, bottomLeft);
